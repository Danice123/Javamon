...
import com.github.Danice123.javamon.SaveFile;
...
//				}
	...
	private void playerActivate() {
		...
		case North:
			...
			if (game.getWorld().getEntity(trueX(), trueY() + 1, getLayer() + 1) != null)
				game.getWorld().getEntity(trueX(), trueY() + 1, getLayer() + 1).activate(this);
			break;
		case South:
			if (game.getWorld().getEntity(trueX(), trueY() - 1, getLayer()) != null)
				game.getWorld().getEntity(trueX(), trueY() - 1, getLayer()).activate(this);
		...
		case East:
			...
			if (game.getWorld().getEntity(trueX() + 1, trueY(), getLayer() + 1) != null)
				game.getWorld().getEntity(trueX() + 1, trueY(), getLayer() + 1).activate(this);
			break;
		case West:
			if (game.getWorld().getEntity(trueX() - 1, trueY(), getLayer()) != null)
				game.getWorld().getEntity(trueX() - 1, trueY(), getLayer()).activate(this);
	...
	public SaveFile createSave() {
		...
		s.x = trueX();
		s.y = trueY();
		s.mapName = game.getWorld().getMapName();
		return s;
	}
	
	public String load(SaveFile s) {
		flag = s.flag;
		pokeData = s.pokeData;
		name = s.name;
		party = s.party;
		face(s.facing);
		setCoords(s.x, s.y, s.layer);
		return s.mapName;
	}
}
